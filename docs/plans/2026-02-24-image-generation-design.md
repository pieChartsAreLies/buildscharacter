...
| Image count per concept | 1 | Only 3 concepts get images (top 3 from 5-10 generated). Agent can retry if needed. |
...
Design Batch Workflow
  ...
  |
  NEW STEPS:
  6. For each top 3: write Imagen prompt from concept
  7. Call generate_design_image -> get image bytes
  8. Call upload_to_r2 -> get public URL
  |
  9. Create Printful product with image URL (bootstrap)
...
LangGraph tool that calls Gemini Imagen API.
...
Returns: base64-encoded PNG image bytes as a string.

Implementation:
- Uses `google.genai.Client` with `settings.google_api_key`
- Calls `client.models.generate_images()` with `imagen-4.0-generate-001`
- `number_of_images=1`
- Returns base64-encoded bytes from `response.generated_images[0].image.imageBytes`
...
- `filename` (str): Desired filename (e.g., "rain-day-sticker.png")

Returns: Public URL string (e.g., `https://{r2_public_url}/designs/rain-day-sticker.png`)
...
Insert image generation step:
> "For each of your top 3 concepts, write a detailed image generation prompt that captures the design. Consider the product type (a sticker design needs different composition than a poster). Generate the image using generate_design_image, then upload it to R2 using upload_to_r2. Use the returned URL when creating the Printful product."
...
| `hobson/src/hobson/workflows/design_batch.py` | Update both prompts with image gen steps + product focus |
| CT 255 `.env` | Add R2 credentials |
| `hobson/pyproject.toml` | Add google-genai, boto3 dependencies |
